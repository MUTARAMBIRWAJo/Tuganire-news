...
export default async function PublicSavedPage() {
  ...
  return (
    ...
    >
      <Card className="border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <CardContent>
          {items.length === 0 ? (
            <p className="text-sm text-slate-600 dark:text-slate-300">You haven't saved any articles yet.</p>
          ) : (
              ...
              {items.map((row: any) => (
                <li key={row.id} className="border-b last:border-b-0 py-3">
                  {row.article ? (
                    ...
                    <div className="text-sm text-slate-600 dark:text-slate-300">Saved item</div>
                  )}
                </li>
              ))}
            </ul>
